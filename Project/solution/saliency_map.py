...
def compute_gradient_saliency_maps(samples: torch.tensor, true_labels: torch.tensor, model: nn.Module):
    ...
    scores = scores.gather(1, true_labels.view(-1, 1))
    scores.backward(torch.ones_like(scores))
    saliency = samples.grad.abs().max(dim=1)[0]
    return saliency
    ######################################################################
...
def main():  # pylint: disable=R0914, R0915
    ...
    for _ in range(6):
        samples, true_labels = next(iter(DataLoader(test_dataset, batch_size=6, shuffle=True)))
        all_samples.append(torch.cat([sample_to_image(s).unsqueeze(0) for s in samples]))
        saliency_maps = compute_gradient_saliency_maps(samples.to(device), true_labels.to(device), model)
    ...
    for idx, (image, saliency_map) in enumerate(zip(all_samples, all_saliency_maps)):
    ...
    saliency_maps_and_images_pairs.savefig(
        os.path.join(FIGURES_DIR, f'{args.dataset}_{args.model}_' f'saliency_maps_and_images_pairs.png')
    )
    ...
    for samples, true_labels in dataloader:
        ...
        saliency_maps = compute_gradient_saliency_maps(fake_samples, fake_labels, model)
        fake_images_saliency_maps.append(saliency_maps.cpu().detach())
        saliency_maps = compute_gradient_saliency_maps(real_samples, real_labels, model)
    ...
    mean_saliency_maps.savefig(os.path.join(FIGURES_DIR, f'{args.dataset}_{args.model}_mean_saliency_maps.png'))
